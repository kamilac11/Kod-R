...
maxim=max(dane_sum$`PRZEBIEG_[km]`)
for (i in 1:nrow(dane_sum)){
  dane_sum[i,which(colnames(dane_sum)=="PRZEBIEG_[km]")]=maxim-dane_sum[i,which(colnames(dane_sum)=="PRZEBIEG_[km]")]       #wczesniej by³ blad bo bralo w petli juz wystumulowana zmienna max(dane_sum[11])-dane_sum[i,11]
}

...
```{r}
library(readxl)
zbior_danych <- read_excel(
  "~/Praca licencjacka/Moje dane_+zrodlo/8_Rozniacych_sie_obiektow.xlsx", 
...
Ostatni etap to graficzne zaprezentowanie wyniku w postaci dendrogramu. W tym celu nale¿y zaimportowaæ bibliotekê factoextra, w której to jest funkcja fviz_dend(x, k = 4, rect = TRUE, main = ""). W miejsce argumentu x nale¿y wpisaæ nazwê obiektu powsta³ego przy pomocy funkcji agnes, k - oznacza iloœæ ró¿nych grup, której znajduj¹ sie najbardziej podobne do siebie obiekty, rect=TRUE - oznacza logiczny warunek na to, ¿e chcemy dodaæ prostok¹ty oddzielaj¹ce poszczególne grupy od siebie, z kolei main to tytu³ wykresu. 
...
```{r}
library(factoextra)
fviz_dend(metoda_najblizszego, k = 4, rect = TRUE, main = "Metoda najbli¿szego s¹siedztwa")
...
Dla tak ma³ego zbioru, dla którego nie wszystkie obiekty znacz¹co ró¿ni¹ siê miêdzy sob¹, nie s¹ zbytnio widoczne ró¿nice uporz¹dkowania nieliniowego. Aczkolwiek w przypadku dendrogramu uzyskanego metod¹ najdalszego s¹siedztwa mo¿na zauwa¿yæ wiêksz¹ odleg³oœæ wi¹zañ ni¿ dla metody najbli¿szego s¹siedztwa, które to obrazuj¹ odleg³oœæ miêdzy grupami. Dodatkowo obiekty charakteryzuj¹ce siê najbardziej korzystnym wartoœci zmiennych, zosta³y pogrupowane w jedn¹ grupê (tu mowa o obiektach o numerach indeksów 2, 3, 4), natomiast w przypadku metody najbli¿szego s¹siedztwa obiekty te zosta³y rozdzielone w osobne grupy.
